...
export default function ChangePasswordModal() {
  ...
  return (
    <>
            ...
            >
              <FontAwesomeIcon icon={faLock} className="mr-2" /> Cambiar Contraseña
            </button>

      <AnimatePresence>
        {isOpen && (
          <motion.div 
            className="fixed inset-0 flex items-center justify-center backdrop-blur-md bg-black/50 z-50"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            <motion.div 
              ...
              exit={{ y: -50, opacity: 0 }}
            >
              <span className="text-2xl font-semibold flex justify-center text-gray-800 mb-4 text-center">Cambiar Contraseña</span>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-2 mt-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition disabled:bg-gray-400"
                  disabled={loading}
                >
                  {loading ? "Cambiando..." : "Cambiar Contraseña"}
                </button>
                <button onClick={() => setIsOpen(false)} type="button" className="w-full mt-2 py-2 btn-cancel bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400">
                  Cancelar
                </button>
              </form>
            </motion.div>
